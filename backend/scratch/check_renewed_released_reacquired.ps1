...
AND EXISTS (
    SELECT 1 FROM tbs_auction_transactions t2 
    WHERE t2.UserId = s.UserId AND t2.RelatedPlayerId = s.PlayerId 
...
"@
    ...
    Write-Output "=========================================================================================="
    Write-Output "    CHECK: MANAGERS WHO RENEWED -> RELEASED -> RE-ACQUIRED (THE TARGET CHEATING CASE)      "
    Write-Output "=========================================================================================="
    Write-Output ("Total cases found in Database: {0}" -f $dt.Rows.Count)
    
    if ($dt.Rows.Count -eq 0) {
        Write-Output ""
        Write-Output ">>> GOOD NEWS: Zero (0) managers have ever exploited this case in the database! <<<"
    } else {
