    ...
    <Box sx={{ pb: 6 }}>
      ...
      >
        ...
        <Autocomplete
          ...
          getOptionLabel={(option) =>
            `${option.userId} ${option.lineName ? `(${option.lineName})` : ""}`
          }
          value={selectedClub}
      ...
      {squadData && squadData.squad
        ? (() => {
            ...
            const gradeSummary =
              (squadData.quotas || []).map((q) => ({
                label: q.gradeName,
                count:
                  (squadData.squad || []).filter(
                    (p) =>
                      (p.playerOvr || 0) >= q.minOVR &&
                      (p.playerOvr || 0) <= q.maxOVR,
                  ).length || 0,
                gradient:
                  GRADE_STYLE_MAP[q.gradeName]?.bg ||
                  GRADE_STYLE_MAP.DEFAULT.bg,
                color:
                  GRADE_STYLE_MAP[q.gradeName]?.color ||
                  GRADE_STYLE_MAP.DEFAULT.color,
              })) || [];
            ...
            return (
              <>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={4}>
                      <Box sx={{ p: 1 }}>
                        ...
                        <Box
                          sx={{ display: "flex", flexWrap: "wrap", gap: 1.2 }}
                        >
                          {gradeSummary.map((g) => (
                            <Box
                ...
                >
                  Team Roster{" "}
                  <Chip
                    label={squadData.squad?.length || 0}
                    size="small"
                    sx={{
                      fontWeight: 1000,
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {squadData.squad?.map((player) => {
                        const gradeInfo = squadData.quotas?.find(
                          (q) =>
                            player.playerOvr >= q.minOVR &&
                            player.playerOvr <= q.maxOVR,
                        );
                        const gradeName = gradeInfo?.gradeName || "E";
                        const gradeStyle =
                          GRADE_STYLE_MAP[gradeName] || GRADE_STYLE_MAP.DEFAULT;
                          ...
                          >
                            ...
                            <TableCell align="center">
                              <Typography variant="body1" fontWeight={1000}>
                                {(player.pricePaid || 0).toLocaleString()}{" "}
                                <Typography
                                  component="span"
                                  variant="caption"
                                  sx={{ opacity: 0.4 }}
                                  ...
                                  >
                                    ...
                                    <Box
                                      sx={{
                                        ...
                                        bgcolor: isTransfer
                                          ? "#0f172a"
                                          : "#431407", // Matching orange-brown tone for Loan
                                        color: "white",
                                        mb: 2,
                                        position: "relative",
                                        overflow: "hidden",
                                      ...
                                      >
                                        ...
                                        {summary?.requiredReserve > 0 && (
                                          <Box textAlign="right">
                                            ...
                                            >
                                              {summary.requiredReserve.toLocaleString()}{" "}
                                              TP
                                            </Typography>
                                          </Box>
                                        )}
        ...
        : !squadLoading && (
            <Box sx={{ textAlign: "center", py: 10 }}>
              ...
              <Typography color="text.secondary">
                Please select a club to view their roster.
              </Typography>
            </Box>
          )}
    </Box>
